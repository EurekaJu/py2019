...
##
##     Filename: MCD4710_T3_2019_A1_SolutionVer1.6_Student_Release.py
##     Created: 20/10/2019
...
def canPlacePole(row, col, pole, workingBoard):
    ...
    (col > 0 and workingBoard[row][col - 1] == pole)or(col < N - 1and workingBoard[row][col + 1] == pole)):
        return False
    return True
...
def setToBoard(my_set, orientations):
    ...
    for i in range(M):
        ...
        for j in range(N):
            s_list.append('E')
        workingBoard.append(s_list)
    set_index = 0
    for row in range(M):
        for col in range(N):
            if workingBoard[row][col] == 'E' and set_length > 0:
                if my_set[set_index] == 0:
                    if orientations[row][col] == 'L':
                        workingBoard[row][col] = '+'
                        workingBoard[row][col + 1] = '-'
                    elif orientations[row][col] == 'T':
                        workingBoard[row][col] = '+'
                        workingBoard[row + 1][col] = '-'
                elif my_set[set_index] == 1:
                    if orientations[row][col] == 'L':
                        workingBoard[row][col] = '-'
                        workingBoard[row][col + 1] = '+'
                    elif orientations[row][col] == 'T':
                    ...
                    if orientations[row][col] == 'L':
                        workingBoard[row][col] = 'X'
                        workingBoard[row][col + 1] = 'X'
                    elif orientations[row][col] == 'T':
                        workingBoard[row][col] = 'X'
                        workingBoard[row + 1][col] = 'X'
                set_length -= 1
                set_index += 1
    return workingBoard
...
def isSolution(positivesColumn, negativesColumn, positivesRow, negativesRow, orientations, workingBoard):
    ...
    for row in range(len(workingBoard)):
        for col in range(len(workingBoard[0])):
            if workingBoard[row][col] != 'X':
                result_set.add(canPlacePole(row, col, workingBoard[row][col], workingBoard))

    for i in range(len(positivesRow)):
        if positivesRow[i] != -1:
            if poleCount('r', i, '+', workingBoard) == positivesRow[i]:
                result_set.add(True)
            else:
                result_set.add(False)
    for i in range(len(positivesColumn)):
        if positivesColumn[i] != -1:
            if poleCount('c', i, '+', workingBoard) == positivesColumn[i]:
                result_set.add(True)
            else:
                result_set.add(False)
    for i in range(len(negativesRow)):
        if negativesRow[i] != -1:
            if poleCount('r', i, '-', workingBoard) == negativesRow[i]:
                result_set.add(True)
            else:
                result_set.add(False)
    for i in range(len(negativesColumn)):
        if negativesColumn[i] != -1:
            if poleCount('c', i, '-', workingBoard) == negativesColumn[i]:
                result_set.add(True)
            else:
                result_set.add(False)
    if len(result_set) == 1 and True in result_set:
        return True
    else:
        return False
...
def bruteforce(positivesColumn, negativesColumn, positivesRow, negativesRow, orientations):
    ...
    for item in all_sol:
        solution = setToBoard(item, orientations)
